 ...
 * </p>
 */
package org.qhn.stateless.threeGitHub.jdbc;

import java.util.List;
import org.qhn.stateless.threeGitHub.jdbc.delegate.CountDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.DeleteDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.FindDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.GetDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.InsertDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.InsertsDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.UpdateDelegate;
import org.qhn.stateless.threeGitHub.jdbc.delegate.UpdatesDelegate;
...
public class JdbcEnhance {
    ...
    public int delete(Class<?> persistentClass, Object primaryKeyValue) throws DataAccessException {
        ...
        DeleteDelegate delegate = new DeleteDelegate(jdbcTemplate, persistentClass,
            primaryKeyValue);
        int rows = delegate.execute();
        delegate = null;//hlep gc.
        return rows;
    ...
    public <T> T get(Class<?> persistentClass, Object primaryKeyValue) throws DataAccessException {
        Assert.notNull(persistentClass, "实体类型不能为空");
        Assert.notNull(primaryKeyValue, "主键不能为空");
        GetDelegate<T> delegate = new GetDelegate<T>(jdbcTemplate, persistentClass,
            primaryKeyValue);
        try {
     ...
     * @param parameters 参数
     */
    public <T> T get(String sql, Class<?> persistentClass, Object... parameters)
     ...
     * @return 实体对象列表
     */
    public <T> List<T> find(SqlBuilder selectSql, Class<?> persistentClass, Object... parameters)
        throws DataAccessException {
        Assert.notNull(persistentClass, "实体类型不能为空");
        Assert.hasText(selectSql.toString(), "sql语句不能为空");
        FindDelegate<List<T>> delegate = new FindDelegate<List<T>>(jdbcTemplate, dialectName,
            persistentClass, selectSql.toString(), parameters);
     ...
     * @return 实体对象列表
     */
    public <T> List<T> find(SqlBuilder selectSql, Class<?> persistentClass, int startRow, int limit,
        ...
        FindDelegate<List<T>> delegate = new FindDelegate<List<T>>(jdbcTemplate, dialectName,
            persistentClass, selectSql.toString(), parameters, null, startRow, limit);
        List<T> list = delegate.execute();
        delegate = null;//hlep gc.
        return list;
    }
     ...
     * @return 实体对象列表
     */
    public <T> List<T> find(String sql, Class<?> persistentClass, Object... parameters)
        throws DataAccessException {
        Assert.notNull(persistentClass, "实体类型不能为空");
        Assert.hasText(sql, "sql语句不能为空");
        FindDelegate<List<T>> delegate = new FindDelegate<List<T>>(jdbcTemplate, dialectName,
            persistentClass, sql, parameters);
        List<T> list = delegate.execute();
        delegate = null;//hlep gc.
        return list;
    }
     ...
     * @return 实体对象列表
     */
    public <T> List<T> find(String sql, Class<?> persistentClass, int startRow, int limit,
        Object... parameters) throws DataAccessException {
        Assert.notNull(persistentClass, "实体类型不能为空");
        Assert.hasText(sql, "sql语句不能为空");
        Assert.isTrue(startRow >= 0, "startRow必须大于等于0");
        Assert.isTrue(limit > 0, "limit必要大于0");
        FindDelegate<List<T>> delegate = new FindDelegate<List<T>>(jdbcTemplate, dialectName,
            persistentClass, sql, parameters, null, startRow, limit);
        List<T> list = delegate.execute();
        delegate = null;//hlep gc.
        return list;
    }
