...
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.ColumnResolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.GeneratedValueResolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.IdResolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.LobResolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.Resolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.TableResolver;
import org.qhn.stateless.threeGitHub.jdbc.metadata.resolver.TransientResolver;
...
public class ElementResolver {
    ...
    public static EntityElement resolve(Class<?> persistentClass) {
        ...
        for (Field field : fields) {
            ...
            Method readMethod = JdbcCommons
                .getReadMethod(persistentClass, mappedSuperclass, fieldName);
            Method writeMethod = JdbcCommons
                .getWriteMethod(persistentClass, mappedSuperclass, fieldName, field.getType());
            ...
            if (!fieldElement.isTransientField()) {
                Assert.notNull(readMethod,
                    "实体:" + persistentClassName + ", 字段：" + fieldName + " 没有get方法");
                Assert.notNull(writeMethod,
                    "实体:" + persistentClassName + ", 字段：" + fieldName + " 没有set方法");
            }
        ...
        Assert
            .notNull(entityElement.getPrimaryKey(), "实体：" + entityElement.getName() + ",必须要注解主键。");
     ...
     * 解析动态实体
     */
    public static DynamicEntityElement resolveDynamic(Class<?> dynamicEntityClass,
        ...
        for (Field field : fields) {
            ...
            String fieldName = field.getName();
            Method readMethod = JdbcCommons
                .getReadMethod(dynamicEntityClass, superclass, fieldName);
            Method writeMethod = JdbcCommons
                .getWriteMethod(dynamicEntityClass, superclass, fieldName, field.getType());
            Assert.notNull(readMethod,
                "实体:" + dynamicEntityClassName + ", 字段：" + fieldName + " 没有get方法");
            Assert.notNull(writeMethod,
                "实体:" + dynamicEntityClassName + ", 字段：" + fieldName + " 没有set方法");
            ...
            dynamicEntityElement
                .addDynamicFieldElements(columnName.toUpperCase(), dynamicFieldElement);
        }
        if (!dynamicMappinged) {
